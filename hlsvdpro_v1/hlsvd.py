...
Functions:
    ...
    hlsvd_v1(data, nsv_sought, dwell_time) -> 6-tuple
...
Example:
    $ python hlvsd.py  (NOT IMPLEMENTED)
...
def hlsvd(signals, nsv_sought, dwell_time, libhlsvd=None):
    """
    This calls the HLSVDPRO original version 1.x library for backwards
    compatibility and comparison. It may be deprecated in the future.

    Args:
        signals (ndarray): an iterable of complex numbers.
        ...
        dwell_time (float): Dwell time in milliseconds for signals data.

        libhlsvd: None (the default) or a string. If None, this function will
            try to find the library on its own. If a string, it should be a
            file name (fully qualified or not, e.g. "hlsvd.dll" or
            "/home/philip/lib/libhlsvd.dylib") that this function can pass to
            ctypes.CDLL().

    Returns:
        tuple: a 6-tuple containing -
            (int), number of singular values found (nsv_found <= nsv_sought)
            (list, floats) the singular values
            (list, floats) the frequencies (in kilohertz)
            (list, floats) the damping factors
            (list, floats) the amplitudes (in arbitrary units)
            (list, floats) the phases (in radians)

            For the first five lists, length == nsv_found and element N of
            ...
            first.  The frequencies and damping factors HAVE been adjusted
            by the dwell time, and the phases converted to degrees.
    ...
    # Arguments are passed as pointers because Fortran passes by reference.
    # Most of these could be passed by value which would make this code easier
    # to read.
    # Passing by value is possible if the VALUE keyword is added to the
    # appropriate parameter declaration in the Fortran code. We don't use it
    # because support for that keyword was added in GFortran 4.3, and we need
    # to use GFortran 4.1 on CentOS 5.11 which is our platform for building
    # Linux wheels.
...
TESTDATA = {"step_size": 0.256,
            ...
            "phas0": [59.67566264,  124.50761197,  156.53854928, -134.60476041, 121.41003743,  120.77262569,  -55.18178104,  115.39431556, 122.81151056,  134.18806697,  149.55942452,  108.06736877, 132.66981625, 92.54605395, 115.24907773, 131.43227559, 89.78270011, 107.68081094, 82.20122509, 96.42674341],
            }
