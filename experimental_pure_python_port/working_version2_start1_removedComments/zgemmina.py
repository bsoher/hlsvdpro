...
#
# BJS - may have been replaced with numpy matrix multiply
#
...
def zgemmina(m, n, k, aaa, bbb, ldb):
    ...
    # got this size from fotran zgemm_ovwr.f zgemmina_python()
    # - but that func not exposed in DLL, so rewrote for zgemmina()
    ldwork = 90145
    WorkArrayType = ctypes.c_double * ldwork
    dwork_fortran = WorkArrayType()
    for i in range(ldwork):
        dwork_fortran[i] = 0.0
...
#                      ctypes.pointer(ldb)
#                     )
    libhlsvd.zgemmina_(ctypes.pointer(m),
                     ctypes.pointer(n),
                     ctypes.pointer(k),
                     ctypes.pointer(aaa_r),
                     ctypes.pointer(aaa_i),
                     ...
                     ctypes.pointer(bbb_fortran),
                     ctypes.pointer(ldb),
                     ctypes.pointer(dwork_fortran),
                     ctypes.pointer(ldwork)
                    )
