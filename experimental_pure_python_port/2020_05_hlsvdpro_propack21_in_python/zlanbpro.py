...
def zlanbpro( m, n, k0, k, aprod, U, ldu, V, ldv, bbb_a, bbb_b, rnorm, options, parm):
    ...
    #print('eps, eps34, epsn, epsn2= ', EPS, EPS34, EPSN, EPSN2 )
    ...
    # big.
    # bjs orig indices = np.zeros(((2 * MAX_SINGULAR_VALUES) + 1,), np.int)
    indices = np.zeros(((2 * k) + 1,), np.int)
...
def dcompute_int(mus, j, delta, eta):
    ...
    if delta >= eta:
        while i < j:
            ...
            indices.append(i)
            # if i == 0:                # bjs found it gave indices a bit more like the fortran
            #     indices.append(i)
            # else:
            #     indices.append(i-1)
...
def dcompute_int_fort(mus, j0, delta, eta):
    """
    I think "int" is short for "intervals". returns indices.
    ...
    if delta >= eta:
        ...
        while i < j:
            ...
            for k in range(i+1,j+1):
                if np.abs(mus[k-1]) > delta:

                    #     find smallest i<k such that for all j=i,..,k, m(j) >= eta
                    for s in range(k, max(i, 0), -1):
                        if np.abs(mus[s-1]) < eta:
                            break

                    #ip = ip + 1
                    indices[ip] = s
                    for i in range(s, j+2):
                        if np.abs(mus[i-1]) < eta:
                            break
...
def dupdate_mu(mus, nus, j, alpha, beta, anorm, eps1):
    """
    ...
    """
    if j == 0:
        # d = eps1 * (sqrt(alpha[j]*alpha[j]+beta[j]*beta[j]) + alpha[0]) + \
        #     eps1 * anorm
        mus[0] = eps1 / beta[0]
        mumax = abs(mus[0])
        ...
        d = eps1 * (const1 + alpha[0]) + eps1 * anorm
        ...
        for k in the_range:
            ...
            d = eps1 * ((const1 + sqrt(alpha[k]*alpha[k]+beta[k-1]*beta[k-1])) + anorm)
            mus[k] = (mus[k] + math.copysign(d, mus[k])) / beta[j]
            mumax = max(mumax, abs(mus[k]))

        mus[j] = beta[j - 1] * nus[j - 1]
        d = eps1 * ((const1 + sqrt(alpha[j]*alpha[j] + beta[j-1]*beta[j-1])) + anorm)
...
def dupdate_nu(mus, nus, j, alpha, beta, anorm, eps1):
    """
    ...
    if j > 0:
        ...
        for k in fortran_range(0, j - 1):
            ...
            d = eps1 * ((sqrt(alpha[k]*alpha[k]+beta[k]*beta[k]) + const1) + eps1)
        ...
        #              (beta[j - 1] * nus[k])
        #     d = eps1 * (sqrt(alpha[k]*alpha[k]+beta[k]*beta[k]) + \
