...
# ndp        n_data_points      
# kmax       kmax               not limited any longer
# lrow       n_rows             # of rows in the critical work matrix 
# mcol       n_columns          # of columns in the critical work matrix 


def hlsvdpro_python(signals, nsv_sought, M=None):

    mode = 'f'          # or 'b' for foward or backward
    xx   = signals
    K    = nsv_sought
    N    = len(xx)
    ...
    if mode == "f":
        ...
        X = sp.linalg.hankel(xx[:M-1:-1], xx[M::-1])

    U, s, Vh = pypropack_python.svdp(X, K, kmax=5*K)
    ...
    r = lapack.zgelss(zeta, signals, cond=-1.0, lwork=lwork, overwrite_a=False, overwrite_b=False)
...
def test():
    ...
    A = [0.54881350 + 0.31179588j, 0.71518937 + 0.69634349j, 0.60276338 + 0.37775184j, 0.54488318 + 0.17960368j,
         0.42365480 + 0.02467873j, 0.64589411 + 0.06724963j, 0.43758721 + 0.67939277j, 0.89177300 + 0.45369684j,
         0.96366276 + 0.53657921j, 0.38344152 + 0.89667129j]
    A = np.array(A)
    ...
    kfound, sing, freq, damp, ampl, phas, u, vt = hlsvdpro_python(A, k, M)
