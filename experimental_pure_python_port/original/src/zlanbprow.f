cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c........1.........2.........3.........4.........5.........6.........7..
c23456789012345678901234567890123456789012345678901234567890123456789012
c
      subroutine zlanbprow_python(ndp, m, n, k0, k, ierr,
     c                            uuu_r, uuu_i, ldu,
     c                            vvv_r, vvv_i, ldv,
     c                            bbb, ldb,
     c                            rnorm, doption, ioption, 
     c                            lambda_r, lambda_i,
     c                            trlambda_r, trlambda_i)


C       subroutine zlanbprow_python(ndp, m, n, k0, k, ierr,
C      c                            uuu_r, uuu_i, ldu,
C      c                            vvv_r, vvv_i, ldv,
C      c                            work1, lwork1,
C      c                            bbb, ldb,
C      c                            bbb1, 
C      c                            workp, lworkp,
C      c                            workq, lworkq,
C      c                            rnorm, doption, ioption, 
C      c                            lambda_r, lambda_i,
C      c                            trlambda_r, trlambda_i)
      implicit none


C     params
      integer ndp, m, n, k, k0, kmax, ierr, ldu, ldv, ldb
      parameter (kmax=50)
      integer ndpmax
      parameter (ndpmax=8192)
C       integer lwork1, lworkp, lworkq, 
      integer lworktotal
      integer info
      real*8 lambda_r(ndp), lambda_i(ndp)
      real*8 trlambda_r(ndp), trlambda_i(ndp)
      real*8 uuu_r(ldu, kmax + 1), uuu_i(ldu, kmax + 1)
      real*8 vvv_r(ldv, kmax + 1), vvv_i(ldv, kmax + 1)
      real*8 rnorm
      real*8 bbb(ldb, 2)        ! dims = (lanmax, 2)
C       real*8 bbb1(ldb, 2)       ! dims = (lanmax, 2)
C       real*8 work1(lwork1), workp(lworkp), workq(lworkq)


c     Local variables.
      integer ips, jps, kps, work_index
      complex*16 uuu(ldu, kmax + 1)
      complex*16 vvv(ldv, kmax)
      complex*16 lambda(ndp)
      complex*16 trlambda(ndp)

      integer ioption(2)
      real*8 doption(3)
      real*8 a_work

      integer, allocatable :: iwork(:)
      real*8, allocatable :: work(:)
      complex*16, allocatable :: zwork(:)


      allocate(iwork(2*kmax+1))
            

C      i = m+n+13*kmax+8*kmax**2+32*m+ndp*kmax
C      lworktotal = lwork1 + (ldb * 2) + (ldb * 2) + lworkp + lworkq
      lworktotal = 295115
      allocate(work(lworktotal))

C       print *, "inside zlanbprow_python"

C       print *, "lworktotal = ", lworktotal

C       i = m+n+max(m,n)+2*kmax+2
      ips = m+n+32*m+7*kmax+2+2*kmax**2+5*ndp
      allocate(zwork(ips))

C       print *, "zwork length = ", ips

C       do ips = 1, lwork1 + 1
C         work(ips) = work1(ips)
C C       PRINT *, "work(", ips "):", work(ips)
C       end do


C       jps = ips
C       do ips = 1, ldb
C           do kps = 1, 2
C C            work_index = jps + kps + ((ips - 1) * ldb) - 1
C C            work(work_index) = bbb(kps, ips)
C C            PRINT *, "bbb(", ips, ",", kps, "):", bbb(ips, kps)
C C           PRINT *, "work(", work_index, "):", work(work_index)
C         end do
C       end do

C       jps = jps + ldb * 2
C       do ips = 1, (ldb * 2)
C           do kps = 1, ldb
C           work(jps + kps + ((ips - 1) * ldb)) = bbb1(kps, ips)
C C       PRINT *, "work(", i "):", work(i)
C         end do
C       end do

C       jps = ips
C       do ips = 1, lworkp
C         work(jps + ips) = workp(ips)
C C       PRINT *, "work(", i "):", work(i)
C       end do

C       jps = ips
C       do ips = 1, lworkq
C         work(jps + ips) = workq(ips)
C C       PRINT *, "work(", i "):", work(i)
C       end do

      do ips = 1, m
        do jps = 1, kmax + 1
          uuu(ips, jps) = dcmplx(uuu_r(ips,jps), uuu_i(ips,jps))
C            PRINT *, "uuu(", ips, jps, "):", uuu(ips, jps)
       end do
      end do

      do ips = 1, n
        do jps = 1, kmax
          vvv(ips,jps) = dcmplx(vvv_r(ips,jps), vvv_i(ips,jps))
C             PRINT *, "zlanbp_python: vvv(",ips,jps,"):", vvv(ips,jps)
       end do
      end do

C       do i = 1, lb
C           bbb(i,j) = dcmplx(bbb_r(i,j), bbb_i(i,j))
C C            PRINT *, "bbb(", i, j, "):", bbb(i,j)
C        end do
      
      do ips = 1, ndp
         lambda(ips) = dcmplx(lambda_r(ips), lambda_i(ips))
C          print *, "lambda(", i, ") = ", lambda(i)
      end do

      do ips = 1, ndp
         trlambda(ips) = dcmplx(trlambda_r(ips), trlambda_i(ips))
C          print *, "trlambda(", i, ") = ", trlambda(i)
      end do

C       print *, "calling zlanbprow, k, k0 = ", k, k0

C       print *, "zlanbprow_python: printing non-zero work"

C       do ips = 1, lworktotal
C           if (work(ips).ne.0.0d0) then 
C              print *, "work(", ips, ") = ", work(ips)
C           end if
C       end do


C     The Fortran code passes work(ib) and work(iwrk) to zlanbprow where I
C     pass bbb and work. The former is dedicated to bbb. The latter is used
C     in a very limited fashion by zlanbprow and is zero-ed early on, so it's
C     pretty clearly doesn't need to be persistent across invocations of
C     zlanbprow(). For that reason I allocate it on the fly here.
C     
C     An important consequence of the above is that, of the arrays that 
C     the Fortran version of zlansvdw represents in the work array, only bbb 
C     is meaningfully modified by zlanbprow().
      call zlanbprow(ndp, m, n, k0, k, uuu, ldu, vvv, ldv, bbb, 
     c               ldb,
     c               rnorm, doption, ioption, work, iwork,
     c               ierr, zwork, lambda, trlambda, 0, 0)

C       print *, "after zlanbprow, k = ", k

C         call zlanbprow(ndp, m, n, jold, j,  U, ldu, V, ldv,
C     c        work(ib),lanmax,rnorm,doption(1),ioption(1),
C     c        work(iwrk), iwork,ierr,  
C     c        zwork(iwrk), lambda,trlambda,planF,planB)

C       print *, "zlanbprow_python: printing non-zero work"
C       do ips = 1, lworktotal
C           if (work(ips).ne.0.0d0) then 
C              print *, "work(", ips, ") = ", work(ips)
C           end if
C       end do


C       print *, "done zlanbprow()"
       do ips = 1, m
          do jps = 1, kmax + 1
C              PRINT *, "uuu(", ips, jps, "):", uuu(ips,jps)
              uuu_r(ips,jps) = REALPART(uuu(ips,jps))
              uuu_i(ips,jps) = IMAGPART(uuu(ips,jps))
         end do
       end do

       do ips = 1, n
          do jps = 1, kmax
C               PRINT *, "vvv(", i, j, "):", vvv(i,j)
              vvv_r(ips,jps) = REALPART(vvv(ips,jps))
              vvv_i(ips,jps) = IMAGPART(vvv(ips,jps))
         end do
       end do


C       do ips = 1, lwork1 + 1
C         work1(ips) = work(ips)
C C       PRINT *, "work1(", i "):", work(i)
C       end do


C       do ips = 1, ldb
C           do kps = 1, 2
C            PRINT *, "bbb(", ips, ",", kps, "):", bbb(ips, kps)
C         end do
C       end do



C       jps = ips - 1
C       print *, "about to copy bbb, jps = ", jps
C       do ips = 1, ldb
C           do kps = 1, 2
C               work_index = jps + ((kps - 1) * ldb) + ips - 1
C               a_work = work(work_index)
C C           print *, "assigning work(", work_index, 
C C      c             ") = ", a_work, "to bbb(", ips, ", ", kps, ")"
C             bbb(ips, kps) = a_work
C C       PRINT *, "bbb1(", i "):", work(i)
C         end do
C       end do

C       jps = jps + (ldb * 2)
C       print *, "about to copy bbb1, jps = ", jps
C       do ips = 1, ldb
C           do kps = 1, 2
C               work_index = jps + ((kps - 1) * ldb) + ips - 1
C               a_work = work(work_index)
C C           print *, "assigning work(", work_index, 
C C      c             ") = ", a_work, "to bbb1(", ips, ", ", kps, ")"
C             bbb1(ips, kps) = a_work
C C       PRINT *, "bbb1(", i "):", work(i)
C         end do
C       end do

C       jps = jps + (ldb * 2)
C       print *, "about to copy workp, jps = ", jps
C       do ips = 1, lworkp
C         workp(ips) = work(jps + ips)
C C       PRINT *, "workp(", i "):", work(i)
C       end do

C       jps = jps + lworkp
C       print *, "about to copy workq, jps = ", jps
C       do ips = 1, lworkq
C         workq(ips) = work(jps + ips)
C C       PRINT *, "workq(", i "):", work(i)
C       end do


C        do ips = 1, ldb
C           do jps = 1, 2
C             PRINT *, "bbb(", ips, jps, "):", bbb(ips, jps)
C           end do
C        end do

C        do ips = 1, m
C           do jps = 1, kmax + 1
C               PRINT *, "uuu(", ips, jps, "):", uuu(ips,jps)
C          end do
C        end do

C      PS - no need to copy lambda and trlambda; they don't change.       

      deallocate(work)
      deallocate(iwork)
      deallocate(zwork)

      end


      subroutine zlanbprow( ndp, m, n, k0, k,  U, ldu, V, ldv, B, ldb,
     c     rnorm, doption, ioption, work, iwork,
     c      ierr,zwork,lambda,trlambda,planF,planB)
c
c
c
c     doption(1) = delta
c     doption(2) = eta
c     doption(3) = anorm
c
c     ioption(1) = cgs
c     ioption(2) = elr
c
c     work  :  double precision array 
c     iwork :  integer array 
c     (C) Rasmus Munk Larsen, Stanford, 1999
c


c     %-----------%
c     | Arguments |
c     %-----------%
      implicit none
      include 'stat.h'
      integer m, n, k0, k, ldb, ldu, ldv, ierr
      integer ips, jps
      integer ioption(*), iwork(*),ndp,PLANF,PLANB
      double precision rnorm,B(ldb,*), doption(*), work(*)
      complex*16  U(ldu,*),V(ldv,*), zwork(*), czero, s, cone,
     c            lambda(*),trlambda(*)
      external aprodw
      integer kmax
      parameter (kmax=50)


c     %------------%
c     | Parameters |
c     %------------%
      logical DEBUG
      integer MGS
      double precision one, zero, FUDGE, kappa
      parameter(one = 1.0d0, zero = 0.0d0, FUDGE = 1.01)
c      parameter (DEBUG=.TRUE., MGS=0, kappa = 0.717)
      parameter (DEBUG=.FALSE., MGS=1, kappa = 0.717)
      parameter (czero=(0.0d0,0.0d0),cone=(1.0d0,0.0d0))
c     %-----------------%
c     | Local variables |
c     %-----------------%
      integer i,j,iv,iu,inu,imu,is,iidx,j0,iy1,iz1
      double precision eps,eps34,epsn2,epsn,sfmin,delta,eta,anorm
      double precision mumax,numax,alpha,beta,a1,b1,amax,anormest
      logical force_reorth,full_reorth,debito
      real t1,t2,t3
      complex*16 zps


c     %----------------------%
c     | External Subroutines |
c     %----------------------%
      external zgetu0w,zreorth,zsafescal,zzero,izero,zcopy,zaxpy
      external dcompute_int,dupdate_nu,dupdate_mu,dzero
      
c     %--------------------%
c     | External Functions |
c     %--------------------%
      double precision dlamch,dznrm2,dlapy2
      double complex zdotc
      external dznrm2,zdotc
      external dlamch,dlapy2

c-------------------- Here begins executable code ---------------------
      call second(t1)

C       print *, "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ zlanbprow() "
C       call FLUSH()

c     %---------------------------------%
c     | Set machine dependent constants |
c     %---------------------------------%
      eps = dlamch('e')
      eps34 = eps**(3d0/4d0)
      epsn = dble(max(m,n))*eps
      epsn2 = sqrt(dble(max(m,n)))*eps
      sfmin = dlamch('s')
      
c      kappa =1.0d0/dsqrt(2.0d0)
      debito = .false.

c     %------------------------%
c     | Set default parameters |
c     %------------------------%
      if (doption(1).lt.zero) then
         delta = sqrt(eps/k)
      else
         delta = doption(1)
      endif
      if (doption(2).lt.zero) then
         eta = eps34/sqrt(dble(k))
      else
         eta = doption(2)
      endif
      if (delta.le.eta .or. delta.eq.zero) then
         full_reorth = .true.
      else
         full_reorth = .false.
      endif
      if (doption(3).gt.zero) then
         anorm = doption(3)

      else if (k0.gt.0) then
         anorm = dlapy2(B(1,1),B(1,2))
         if (anorm.le.zero) then
            ierr = -1
            goto 9999         
         endif
      else
         anorm = zero
      endif
c               delta =3.107106895105165e-09
c              eta = 3.792855096563922e-13

c     %---------------------%
c     | Get starting vector |
c     %---------------------%

C       print *, "inside zlanbprow (fortran) s= ", s
C       call FLUSH()

C       do i = 1, m
C         do j = 1, kmax + 1
C           if (U(i,j).ne.(0.0d0,0.0d0)) then 
C               PRINT *, "U(", i, j, "):", U(i,j)
C             end if 
C             call FLUSH()
C        end do
C       end do

C       do i = 1, ldv
C         do j = 1, kmax
C           if (V(i,j).ne.(0.0d0,0.0d0)) then 
C               PRINT *, "V(", i, j, "):", V(i,j)
C             end if 
C             call FLUSH()
C        end do
C       end do

C       do i = 1, ldb
C         do j = 1, 2
C           PRINT *, "B(", i, j, "):", B(i,j)
C        end do
C       end do
C       call FLUSH()

C       do i = 1, ndp
C           PRINT *, "lambda(", i, "):", lambda(i)
C       end do

C       do i = 1, ndp
C           PRINT *, "trlambda(", i, "):", trlambda(i)
C       end do

C       print *, "ndp, m, n = ", ndp, m, n
C       print *, "k0, k = ", k0, k
C       print *, "U(1,k0+1) = ", U(1,k0+1)
C       print *, "ldu, ldv, ldb = ", ldu, ldv, ldb
C       print *, "anorm = ", anorm
C       print *, "rnorm = ", rnorm
C       print *, "doption: ", doption(1), doption(2), doption(3)
C       print *, "ioption: ", ioption(1), ioption(2)

C      Also print work, iwork,zwork


C       print *, "stifle!"

      if (rnorm .eq. zero) then
c         call zgetu0('n',m, n, k0, 3, U(1,k0+1) , rnorm, U,ldu, aprod,
c     c         ierr, ioption(1), anormest,zwork,lambda,trlambda)
c         anorm = max(anorm,anormest)
C          print *, "rnorm is zero; calling zgetu0w()"
         call zgetu0w('n',ndp,m, n, k0, 3, U(1,k0+1) , rnorm, U,ldu, 
     c         ierr, ioption(1), anormest,zwork,lambda,
     c           trlambda,planF,planB) ! it was zwork instead of zwork(is) ! diana

ccc         call zgetu0('n',m,n,0,1,U,rnorm,U,ldu,
ccc     c        ierr,ioption(1),anorm,zwork(iwrk),lambda,
ccc     c        trlambda,wsave)

c%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
         anorm = max(anorm,anormest)
      endif

C       print *, "rnorm = ", rnorm, ", anorm = ", anorm, ", anormest = ",
C      c         anormest


c     %------------------------------%
c     | Set pointers into work array |
c     %------------------------------%
      iu = 1
      iv = iu+m
      imu = iv+n
c      iv = iu+ndp
c      imu = iv+ndp
      inu = imu+k+1
      is = inu+k+1
      iz1 = is+3*ndp
      iy1 = iz1+ndp
      iidx = 1
      call dzero(m+n+max(m,n)+2*k+2,work,1)
      call zzero(m+n+max(m,n)+2*k+2,zwork,1)
      call izero(k,iwork,1)

c     %---------------------------%
c     | Prepare Lanczos iteration |
c     %---------------------------%
c      write(*,*)k0,'******************************** k0'

      if (k0.eq.0) then
C          print *, "zlanbprow: k0 is zero"
         amax = zero
         alpha = zero
         beta = rnorm
         force_reorth = .false.
c     %-----------------------------------------------%
c     | Compute ||A x|| / ||x|| for a random vector x |
c     | to make it less likely that ||A|| is grossly  |
c     | underestimated.                               |
c     %-----------------------------------------------%
         if (n.gt.m) then      
C              print *, "zlanbprow: n > m"
c            call zgetu0('n',m,n,0,1,zwork(iu),s,U,ldu,aprod,
c     c           ierr,ioption(1),anormest,zwork(is),lambda,
c     c           trlambda,wsave)
            call zgetu0w('n',ndp,m,n,0,1,zwork(iu),s,U,ldu,
     c           ierr,ioption(1),anormest,zwork(is),lambda,
     c           trlambda,planF,planB)

C             do i=iu, iu + m
C                 print *, "zwork(", i, ") = ", zwork(i)
C             end do
C             print *, "s = ", s
         else  
C              print *, "zlanbprow: n <= m"

C               do i = 1, n
C                 do j = 1, kmax
C                   PRINT *, "V(", i, j, "):", V(i,j)
C                end do
C               end do

c            call zgetu0('y',m,n,0,1,zwork(iv),s,V,ldv,aprod,
c     c           ierr,ioption(1),anormest,zwork(is),lambda,
c     c           trlambda,wsave)
C            print *, "before zgetu0w 531, s = ", s
C           Beware of some sneakiness here. s is declared as a complex
C           but passed to zgetu0w() in a param slot that accepts a 
C           real. The result is that zgetu0w() overwrites the real 
C           portion of s but leaves the imaginary portion untouched.
C           Is this behavior a Fortran feature or just a compiler quirk?
C           Is this type mismatch intentional or a bug?
C             print *, "before zgetu0w 531"
C              do ips = 1, 10
C                 jps = is + ips - 1
C                 print *, "zwork(", jps, ") = ", zwork(jps)
C              end do
C              print *, "-"
            call zgetu0w('t',ndp,m,n,0,1,zwork(iv),s,V,ldv,
     c           ierr,ioption(1),anormest,zwork(is),lambda,
     c           trlambda,planF,planB)
C             print *, "after zgetu0w 531"
C           call EXIT(0)
C              do ips = 1, 10
C                 jps = is + ips - 1
C                 print *, "zwork(", jps, ") = ", zwork(jps)
C              end do
C              print *, "-"
C             print *, "s = ", s, "anormest = ", anormest


C             do i=iv, iv + m
C                 print *, "zwork(", i, ") = ", zwork(i)
C             end do
C             print *, "s = ", s
         endif


C           print *, "ndp, m, n = ", ndp, m, n
C           print *, "k0, k = ", k0, k
C           print *, "U(1,k0+1) = ", U(1,k0+1)
C           print *, "ldu, ldv, ldb = ", ldu, ldv, ldb
C           print *, "anorm = ", anorm
C           print *, "rnorm = ", rnorm
C           print *, "planF, planB = ", planF, planB
C           print *, "doption: ", doption(1), doption(2), doption(3)
C           print *, "ioption: ", ioption(1), ioption(2)

C         print *, "banana"


         anorm = max(anorm,FUDGE*anormest)
C          print *, "max anorm = ", anorm
         j0 = 1
         if (beta.ne.zero) then
            call donothing()
C             print *, "beta = ", beta, "; calling zsafescal w/k0=", k0
            call zsafescal(m,beta,U(1,k0+1))
C             do ips = 1, m
C               PRINT *, "U(", ips, k0 + 1, "):", U(ips, k0 + 1)
C             end do


         endif
         call zcopy(m,U(1,k0+1),1,zwork(iu),1)

         work(imu) = one
         work(inu) = one
         zwork(imu) = cone
         zwork(inu) = cone


      else
C        k0 != 0
         do ips = 1, 51
C             print *, "B(", ips, ", 1) = ", B(ips, 1)
         end do
C         print *, "boom, k0 = ", k0

         alpha = B(k0,1)
         call zcopy(n,V(1,k0),1,zwork(iv),1)
         beta = rnorm
         force_reorth = .true.


         if (k0.lt.k .and. beta*delta.lt.anorm*eps) then
            full_reorth = .true.
            ierr = k0
         endif
         iwork(iidx) = 1
         iwork(iidx+1) = k0
         iwork(iidx+2) = k0+1

         call second(t2)
  
       
c         if (MGS.eq.0) then
c            call zreorth(m,k0,U,ldu,U(1,k0+1),s,iwork(iidx),kappa,
c     c           zwork(is),ioption(1))
c         else
c            call zreorth2(m,k0,U,ldu,U(1,k0+1),s,iwork(iidx))
C                print *, "before zreorth2 624"
            call zreorth2(m,k0,U,ldu,U(1,k0+1),rnorm,iwork(iidx))
c         endif
         call second(t3)
         treorthu = treorthu+(t3-t2)

         call dset_mu(k0,work(imu),iwork(iidx),epsn2)


c     %--------------------------------------%
c     | Estimate ||B||_2^2 as ||B^T * B||_1  |   
c     %--------------------------------------%

c
c         beta = s*beta
c

c*******************************
c         beta = dble(s)*beta
c*******************************
         B(k0,2) = beta
         amax = zero
         do j=1,k0
C            print  *,"631, k0 = ", k0
            amax = max(amax,B(j,1),B(j,2))
            if (j.eq.1) then
               anorm = max(anorm,FUDGE*alpha)
            else if (j.eq.2) then
               a1 = B(1,2)/amax
               a1 = FUDGE*amax*sqrt((B(1,1)/amax)**2 + a1**2 +
     c              B(2,1)/amax*a1)
               anorm = max(anorm,a1)
            else
               a1 = B(j-1,1)/amax
               b1 = B(j-1,2)/amax 
               a1 = FUDGE*amax*sqrt( a1**2 + b1**2 +
     c              a1*B(j-2,2)/amax + B(j,1)/amax*b1)
               anorm = max(anorm,a1)
            endif
            work(imu+j-1) = epsn2
C            print *, "648 setting nus(", inu+j-1, ") = ", epsn2
            work(inu+j-1) = epsn2
         enddo
         j0 = k0+1
         call zcopy(m,U(1,k0+1),1,zwork(iu),1)
      endif
      numax = zero
      mumax = zero


c     %-------------------------------------------%
c     | Start Lanczos bidiagonalization iteration |
c     %-------------------------------------------%            
      if (DEBUG) then
         call flush(6)
      endif

C       print *, "Starting Lanczos bidiagonalization iteration, j0 = ", j0
C       print *, "alpha = ", alpha



C       do ips = 1, m
C         do jps = 1, kmax + 1
C           PRINT *, "U(", ips, jps, "):",  U(ips,jps)
C        end do
C       end do
C       print *, "------"


      do j=j0,k
C         print *, "------------------------------------------"
C         print *, "                        Lanczos iteration, j = ", j



        if (DEBUG .and. mod(j,10).eq.0) then
c           write (*,*) 'alpha, beta = ',alpha,beta
           call flush(6)
        endif

        if (j.eq.1) then
           call second(t2)
c%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


C             do ips =iu, iu + m
C                 print *, "zwork(", ips, ") = ", zwork(ips)
C             end do

C             do ips =iv, iv + n
C                 print *, "zwork(", ips, ") = ", zwork(ips)
C             end do

C             print *, "about to call aprodw"
C             print *, "iu, iv, iz1, iy1 = ", iu, iv, iz1, iy1 

C             do ips = 1, 90000
C                 print *, "zwork(", ips, ") = ", zwork(ips)
C             end do
C             do ips =iv, iv + n
C                 print *, "zwork(", ips, ") = ", zwork(ips)
C             end do

C             print *, "before aprodw 711"
C              do ips = 1, 10
C                 jps = iu + ips - 1
C                 print *, "zwork(", jps, ") = ", zwork(jps)
C              end do
C              print *, "-"
C              do ips = 1, 10
C                 jps = iv + ips - 1
C                 print *, "zwork(", jps, ") = ", zwork(jps)
C              end do
C              print *, "-"
C              do ips = 1, 10
C                 jps = iz1 + ips - 1
C                 print *, "zwork(", jps, ") = ", zwork(jps)
C              end do
C              print *, "-"
C              do ips = 1, 10
C                 jps = iy1 + ips - 1
C                 print *, "zwork(", jps, ") = ", zwork(jps)
C              end do
C              print *, "-"           
             call aprodw('t',ndp,m,n,zwork(iu),zwork(iv),zwork(iz1),
     c                   zwork(iy1),lambda,trlambda,
     c                   planF,planB )
C             call zwrite("1f.txt", m, zwork(iv))
C             call EXIT(0)


C             print *, "after aprodw 711"
C              do ips = 1, 10
C                 jps = iu + ips - 1
C                 print *, "zwork(", jps, ") = ", zwork(jps)
C              end do
C              print *, "-"
C              do ips = 1, 10
C                 jps = iv + ips - 1
C                 print *, "zwork(", jps, ") = ", zwork(jps)
C              end do
C              print *, "-"
C              do ips = 1, 10
C                 jps = iz1 + ips - 1
C                 print *, "zwork(", jps, ") = ", zwork(jps)
C              end do
C              print *, "-"
C              do ips = 1, 10
C                 jps = iy1 + ips - 1
C                 print *, "zwork(", jps, ") = ", zwork(jps)
C              end do
C              print *, "-"

C             do ips =iv, iv + n
C                 print *, "zwork(", ips, ") = ", zwork(ips)
C             end do

C             do ips = 1, 90000
C                 print *, "zwork(", ips, ") = ", zwork(ips)
C             end do

C             do ips =iu, iu + m
C                 print *, "zwork(", ips, ") = ", zwork(ips)
C             end do

C             do ips =iv, iv + n
C                 print *, "zwork(", ips, ") = ", zwork(ips)
C             end do



c%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            call second(t3)
            tmvopx = tmvopx + (t3-t2)
            nopx = nopx+1
            alpha = dznrm2(n,zwork(iv),1)
            anorm = max(anorm,FUDGE*alpha)
C             print *, "alpha (e norm) = ", alpha, ", anorm = ", anorm

         else
            
c     %---------------------------------------------%
c     | alpha_{j} v_{j} = A'*u_{j} - beta_{j} v_{j} |
c     %---------------------------------------------%
            call second(t2)
c%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

C             print *, "before aprodw 748"
C              do ips = 1, 10
C                 jps = iu + ips - 1
C                 print *, "zwork(", jps, ") = ", zwork(jps)
C              end do
C              print *, "-"
C              do ips = 1, 10
C                 jps = is + ips - 1
C                 print *, "zwork(", jps, ") = ", zwork(jps)
C              end do
C              print *, "-"
C              do ips = 1, 10
C                 jps = iz1 + ips - 1
C                 print *, "zwork(", jps, ") = ", zwork(jps)
C              end do
C              print *, "-"
C              do ips = 1, 10
C                 jps = iy1 + ips - 1
C                 print *, "zwork(", jps, ") = ", zwork(jps)
C              end do
C              print *, "-"
            call aprodw('t',ndp,m,n,zwork(iu),zwork(is),zwork(iz1),
     c                   zwork(iy1),lambda,trlambda,
     c                     planF,planB )
C             print *, "after aprodw 748"
C              do ips = 1, 10
C                 jps = iu + ips - 1
C                 print *, "zwork(", jps, ") = ", zwork(jps)
C              end do
C              print *, "-"
C              do ips = 1, 10
C                 jps = is + ips - 1
C                 print *, "zwork(", jps, ") = ", zwork(jps)
C              end do
C              print *, "-"
C              do ips = 1, 10
C                 jps = iz1 + ips - 1
C                 print *, "zwork(", jps, ") = ", zwork(jps)
C              end do
C              print *, "-"
C              do ips = 1, 10
C                 jps = iy1 + ips - 1
C                 print *, "zwork(", jps, ") = ", zwork(jps)
C              end do
C              print *, "-"


c%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            call second(t3)
            tmvopx = tmvopx + (t3-t2)
            nopx = nopx+1

            do i=0,n-1
               zwork(iv+i) = zwork(is+i) - beta*zwork(iv+i)
            enddo

c     %------------------------------------%
c     | Extended local reorthogonalization |
c     %------------------------------------%
            alpha = dznrm2(n,zwork(iv),1)
            call second(t2)
            if (j.gt.1 .and. ioption(2).gt.0 .and.
     c           alpha.lt.kappa*beta) then
               do i=1,2
C                   do ips = 1, n
C                     print *, "647 V(", ips, ", ", j-1, " = ", V(ips,j-1)
C                   end do
C                   do ips = 1, n
C                     jps = ips + iv - 1
C                     print *, "647 zwork(", jps, " = ", zwork(jps)
C                   end do
                  s = zdotc(n,V(1,j-1),1,zwork(iv),1)
C                    print *, "vdot 647, s = ", s


                  call zaxpy(n,-s,V(1,j-1),1,zwork(iv),1)
                  if (beta .ne. zero) then
                     beta = beta + dble(s)
                     B(j-1,2) = beta
                  endif
                  s = dznrm2(n,zwork(iv),1)
c                  if (dsqrt(dble(conjg(s)*s)) .ge. kappa*alpha) goto 10 
                   if (dble(s) .ge. kappa*alpha) goto 10                
               enddo
 10            continue
C               print *, "789 setting nus(", inu+j-2, ") = ", eps
               work(inu+j-2) = eps
c               alpha =dsqrt( dble(conjg(s)*s))
               alpha =dble(s)
            endif
            call second(t3)
            telrv = telrv + (t3-t2)

            B(j,1) = alpha
            amax = max(amax,alpha)
c     %---------------------------%
c     | Update estimate of ||A||_2 |         
c     %---------------------------%
            if (j.eq.2) then
               a1 = B(1,2)/amax
               a1 = FUDGE*amax*sqrt((B(1,1)/amax)**2 + a1**2 +
     c              B(2,1)/amax*a1)
            else
               a1 = B(j-1,1)/amax
               b1 = B(j-1,2)/amax 
               a1 = FUDGE*amax*sqrt( a1**2 + b1**2 +
     c              a1*B(j-2,2)/amax + B(j,1)/amax*b1)
            endif
            anorm = max(anorm,a1)
         endif


C       do ips = 1, m
C         do jps = 1, kmax + 1
C             if (U(ips,jps).ne.(0.0d0,0.0d0)) then
C                PRINT *, "U(", ips, jps, "):",  U(ips,jps)
C             end if
C        end do
C       end do
C       print *, "------"

C           print *, "before Update the nu recurrence, j = ", j
C           do ips = iu, iu + m
C             if (zwork(ips).ne.(0.0d0,0.0d0)) then
C                PRINT *, "zwork(", ips, "):",  zwork(ips)
C             end if
C           end do
C           print *, "------"

C         do ips = iv, iv + n
C             print *, "zwork(", ips, ") = ", zwork(iv)
C         end do

c     %--------------------------%
c     | Update the nu recurrence |
c     %--------------------------%
         if (.not.full_reorth .and. alpha.ne.zero) then
C            print *, "before dupdate_nu(), numax = ", numax
C             if (j.eq.3) then 
C                do ips = inu, inu + 10
C                  PRINT *, "nu(", ips, "):", work(ips)
C                end do
C                do ips = imu, imu + 10
C                  PRINT *, "mu(", ips, "):", work(ips)
C                end do
C             end if
            call dupdate_nu(numax,work(imu),work(inu),j,
     c           B(1,1),B(1,2),anorm)
C             print *, "after dupdate_nu, numax = ", numax
C             if (j.eq.3) then 
C                do ips = inu, inu + 10
C                  PRINT *, "nu(", ips, "):", work(ips)
C                end do
C             end if
         endif


c     %------------------------------%
c     | Reorthogonalize if necessary |
c     %------------------------------%
         if ( .true. .and.
     c        (full_reorth .or. numax.gt.delta .or. force_reorth) 
     c        .and. alpha.ne.zero) then
C             print *, "Reorthogonalizing..."
            if (full_reorth .or. eta.eq.zero) then
C                print *, "865 setting indices"
               iwork(iidx) = 1
               iwork(iidx+1) = j-1
               iwork(iidx+2) = j
            else if (.not. force_reorth) then
C                 print *, "before dcompute_int 981"
                call dcompute_int(work(inu),j-1,delta,eta,iwork(iidx))
            endif
            call second(t2)

            if (MGS.eq.0) then
               call zreorth(n,j-1,V,ldv,zwork(iv),alpha,iwork(iidx),
     c              kappa,zwork(is),ioption(1))
            else
C                print *, "before zreorth2 990"
               call zreorth2(n,j-1,V,ldv,zwork(iv),alpha,iwork(iidx))
            endif
            call second(t3)
            treorthv = treorthv+(t3-t2)


C            print *, "before dset_mu 883"
C             do ips = inu, inu + 10
C               PRINT *, "nu(", ips, "):", work(ips)
C             end do
C             do ips = iidx, iidx + 5
C               PRINT *, "iwork(", ips, "):", iwork(ips)
C             end do
            call dset_mu(j-1,work(inu),iwork(iidx),epsn2)
C             print *, "after dset_mu 883"
C             do ips = inu, inu + 10
C               PRINT *, "nu(", ips, "):", work(ips)
C             end do
            numax = eta
            if (force_reorth) then
               force_reorth = .false.
            else
               force_reorth = .true.
            endif
         endif

C           print *, "after Reorthogonalize if necessary, j = ", j
C           do ips = iu, iu + m
C             if (zwork(ips).ne.(0.0d0,0.0d0)) then
C                PRINT *, "zwork(", ips, "):",  zwork(ips)
C             end if
C           end do
C           print *, "------"


          
c     %-----------------------------------------------%
c     | Check whether an invariant subspace was found |
c     %-----------------------------------------------% 
C          print *, "Checking for invariant subspace..."
         if (alpha .lt. anorm*epsn .and. j.lt.k) then
C             print *, "alpha = ", alpha, ", anorm = ", anorm, 
C      c               ", anorm * epsn = ", anorm * epsn
            rnorm = alpha
            alpha = zero
c     %------------------------------------------------%
c     | Try to build and orthogonal subspace, starting |
c     | with a random vector.                          |
c     %------------------------------------------------%
            call zgetu0w('t',ndp, m, n, j-1, 3, zwork(iv), alpha, V, 
     c             ldv, ierr,ioption(1),anormest,
     c           zwork(is),lambda,trlambda,planF,planB)
            if (alpha .eq. zero) then
               k = j-1
               ierr = -j
               goto 9999
            else
               nrstrt = nrstrt + 1
               call donothing()
               call zsafescal(n,alpha,zwork(iv))
               alpha = zero
               force_reorth = .true.
               if (delta.gt.zero) then
                  full_reorth = .false.
               endif
            endif
         else if (j.gt.1 .and. .not. full_reorth .and. j.lt.k .and.
     c           (delta*alpha .lt. anorm*eps)) then
c            full_reorth = .true.
c            full_reorth = .true.
C             print *, "setting ierr = ", j
            ierr = j
         endif            
         B(j,1) = alpha

         if (alpha.ne.zero) then
            call donothing()
C             print *, "alpha = ", alpha, "; calling zsafescal()"
C             do ips =iv, iv + n
C                 print *, "zwork(", ips, ") = ", zwork(ips)
C             end do
             call zsafescal(n,alpha,zwork(iv))
C             print *, "after zsafescal()"
C             do ips =iv, iv + n
C                 print *, "zwork(", ips, ") = ", zwork(ips)
C             end do
         endif
         call zcopy(n,zwork(iv),1,V(1,j),1)
C          do ips =1,n
C             print *, "V(", ips, ", ", j, ") = ", V(ips, j)
C          end do
 
c     %------------------------------------------------%
c     | beta_{j+1} u_{j+1} = A*v_{j} - alpha_{j} u_{j} |
c     %------------------------------------------------%
         call second(t2)

             
C          print *, "before aprodw 982"
C          do ips = 1, 10
C             jps = iv + ips - 1
C             print *, "zwork(", jps, ") = ", zwork(jps)
C          end do
C          print *, "-"
C          do ips = 1, 10
C             jps = is + ips - 1
C             print *, "zwork(", jps, ") = ", zwork(jps)
C          end do
C          print *, "-"
C          do ips = 1, 10
C             jps = iz1 + ips - 1
C             print *, "zwork(", jps, ") = ", zwork(jps)
C          end do
C          print *, "-"
C          do ips = 1, 10
C             jps = iy1 + ips - 1
C             print *, "zwork(", jps, ") = ", zwork(jps)
C          end do
C          print *, "-"         
           call aprodw('n',ndp,m,n,zwork(iv),zwork(is),zwork(iz1),
     c                   zwork(iy1),lambda,trlambda,
     c                       planF,planB )
C          print *, "after aprodw 982"
C          if (j.eq.9) then
C             call zwrite("8f.txt", n, zwork(iv))
C          end if

C          do ips = 1, 10
C             jps = iv + ips - 1
C             print *, "zwork(", jps, ") = ", zwork(jps)
C          end do
C          print *, "-"
C          do ips = 1, 10
C             jps = is + ips - 1
C             print *, "zwork(", jps, ") = ", zwork(jps)
C          end do
C          print *, "-"
C          do ips = 1, 10
C             jps = iz1 + ips - 1
C             print *, "zwork(", jps, ") = ", zwork(jps)
C          end do
C          print *, "-"
C          do ips = 1, 10
C             jps = iy1 + ips - 1
C             print *, "zwork(", jps, ") = ", zwork(jps)
C          end do
C          print *, "-"         
         call second(t3)
         tmvopx = tmvopx + (t3-t2)
         nopx = nopx+1

C           print *, "before * 2 Extended local reorth, j = ", j
C           print *, "alpha = ", alpha
C           print *, "iu = ", iu
C           print *, "is = ", is

C           do ips = iu, iu + m
C             if (zwork(ips).ne.(0.0d0,0.0d0)) then
C                PRINT *, "zwork(", ips, "):",  zwork(ips)
C             end if
C           end do
C           print *, "------"

         do i=0,m-1
C             if (i.lt.10) then
C               print *, "zwork(", iu + i, ") = ", zwork(iu+i)
C               print *, "zwork(", is + i, ") = ", zwork(is+i)
C             end if
            zwork(iu+i) = zwork(is+i) - alpha*zwork(iu+i)
C             if (i.lt.10) then
C               print *, "zwork(", iu + i, ") = ", zwork(iu+i)
C             end if
         enddo
         
C           print *, "before Extended local reorth, j = ", j
C           do ips = iu, iu + m
C             if (zwork(ips).ne.(0.0d0,0.0d0)) then
C                PRINT *, "zwork(", ips, "):",  zwork(ips)
C             end if
C           end do
C           print *, "------"



c     %------------------------------------%
c     | Extended local reorthogonalization |
c     %------------------------------------%
         call second(t2)
         do i=1,ioption(2)
C           PS - call to zdotc() does not modify U
C             do ips =1,m
C                 print *, "vdot input[", ips, "] = ", U(ips, j)
C             end do
C             do ips =1,m
C                 print *, "vdot input[", ips, "] = ", zwork(ips)
C             end do


            s = zdotc(m,U(1,j),1,zwork(iu),1)
C             print *, "vdot s = ", s

C           PS - I don't think this call to zaxpy() modifies U
            call zaxpy(m,-s,U(1,j),1,zwork(iu),1)
            if (alpha .ne. zero) then
               alpha = alpha + dble(s)

               B(j,1) = alpha
            endif
            work(imu+j-1) = eps
         enddo
         call second(t3)
         telru = telru + (t3-t2)

         beta= dznrm2(m,zwork(iu),1)

C          print *, "beta = ", beta

         B(j,2) = beta

         amax = max(amax,beta)
      
c     %---------------------------%
c     | Update estimate of ||A||_2 |         
c     %---------------------------%
         if (j.eq.1) then
C            print *, "dlapy2 args = ", B(1,1), ", ", B(1,2)
            a1 = dlapy2(B(1,1), B(1,2))
         else
            a1 = B(j,1)/amax
            a1 = amax*sqrt(a1**2 + (B(j,2)/amax)**2 +
     c           a1*B(j-1,2)/amax)
         endif      
C          print *, "a1 = ", a1
         anorm=max(anorm,a1)

C           print *, "before Update the mu recurrence, j = ", j
C           do ips = iu, iu + m
C             if (zwork(ips).ne.(0.0d0,0.0d0)) then
C                PRINT *, "zwork(", ips, "):",  zwork(ips)
C             end if
C           end do
C           print *, "------"

c     %--------------------------%
c     | Update the mu recurrence |
c     %--------------------------%
         if (.not.full_reorth .and. beta.ne.zero) then
C             print *, "before dupdate_mu() 967, anorm = ", anorm
C             do ips = 1, 10
C                 jps = imu + ips - 1
C                PRINT *, "mu work(", jps, ") = ", work(jps)
C             end do
C             do ips = 1, 10
C                 jps = inu + ips - 1
C                PRINT *, "nu work(", jps, ") = ", work(jps)
C             end do
C             do ips = 1, 10
C                PRINT *, "B(", jps, ", 1) = ", B(ips, 1)
C             end do
C             do ips = 1, 10
C                PRINT *, "B(", jps, ", 2) = ", B(ips, 2)
C             end do
            call dupdate_mu(mumax,work(imu),work(inu),j,B(1,1),
     c           B(1,2),anorm)
C             print *, "after dupdate_mu() 967, mumax = ", mumax
C             do ips = 1, 10
C                PRINT *, "mu work(", imu + ips - 1, ") = ", 
C      c                  work(imu + ips - 1)
C             end do
         endif

c     %--------------------------------------%
c     | Reorthogonalize u_{j+1} if necessary |
c     %--------------------------------------%
C          print *, "full_reorth = ", full_reorth
C          print *, "force_reorth = ", force_reorth
C          print *, "mumax = ", mumax, ", delta = ", delta
         if ( .true. .and.
     c        (full_reorth .or. mumax.gt.delta .or. force_reorth) 
     c        .and. beta.ne.zero) then
C             print *, "Reorthogonalizing u_{j+1}"
            if (full_reorth .or. eta.eq.zero) then
C                print *, "1109 setting indices"
               iwork(iidx) = 1
               iwork(iidx+1) = j
               iwork(iidx+2) = j+1
            else if (.not. force_reorth) then
C                 print *, "before dcompute_int 1278"
               call dcompute_int(work(imu),j,delta,eta,iwork(iidx))
            else
               do i=1,j+1
                  if (iwork(iidx+i-1).eq.j) then
C                      print *, "1118 setting indices"
                     iwork(iidx+i-1) = j+1
                     goto 25
                  endif
               enddo
            endif

 25         call second(t2)

c 25         if (MGS.eq.0) then
c 25       call zreorth(m,j,U,ldu,zwork(iu),beta,iwork(iidx),
c     c              kappa, zwork(is),ioption(1))
c            else
C                print *, "before zreorth2()"
C               do ips = 1, m
C                 do jps = 1, kmax + 1
C                   PRINT *, "U(", ips, jps, "):",  U(ips,jps)
C                end do
C               end do
C               print *, "------"
C                print *, "before zreorth2 1303"
               call zreorth2(m,j,U,ldu,zwork(iu),beta,iwork(iidx))
c            endif
            call second(t3)
            treorthu = treorthu+(t3-t2)
            
C            print *, "before dset_mu() 1146"
C             do ips = 1, 10
C                PRINT *, "mu work(", imu + ips - 1, ") = ", 
C      c                  work(imu + ips - 1)
C             end do
            call dset_mu(j,work(imu),iwork(iidx),epsn2)
C            print *, "after dset_mu() 1146"
C             do ips = 1, 10
C                PRINT *, "mu work(", imu + ips - 1, ") = ", 
C      c                  work(imu + ips - 1)
C             end do
            mumax = eta
            if (force_reorth) then
               force_reorth = .false.
            else
               force_reorth = .true.
            endif
         endif

C           print *, "checking for invariant subspace, j = ", j
C           do ips = 1, m
C             do jps = 1, kmax + 1
C                 if (U(ips,jps).ne.(0.0d0,0.0d0)) then
C                    PRINT *, "U(", ips, jps, "):",  U(ips,jps)
C                 end if
C            end do
C           end do
C           print *, "------"

C           print *, "checking for invariant subspace, j = ", j
C           do ips = iu, iu + m
C             if (zwork(ips).ne.(0.0d0,0.0d0)) then
C                PRINT *, "zwork(", ips, "):",  zwork(ips)
C             end if
C           end do
C           print *, "------"


c     %-----------------------------------------------%
c     | Check whether an invariant subspace was found |
c     %-----------------------------------------------%
         if (beta .lt. anorm*epsn .and. j.lt.k) then
            rnorm = beta
            beta = zero
c     %------------------------------------------------%
c     | Try to build an orthogonal subspace, starting |
c     | with a random vector.                          |
c     %------------------------------------------------%
            call zgetu0w('n',ndp, m, n, j, 3, zwork(iu), beta, U, ldu, 
     c             ierr,ioption(1),anormest,zwork(is),
     c             lambda,trlambda,planF,planB)
            if (beta .eq. zero) then
               k = j
               ierr = -j
               goto 9999
            else
               nrstrt = nrstrt + 1
               call donothing()
               call zsafescal(n,beta,zwork(iu))
               beta = zero
               force_reorth = .true.
               if (delta .gt. zero) then
                  full_reorth = .false.
               endif
            endif
         else if (.not.full_reorth .and. j.lt.k .and. 
     c           (delta*beta .lt. anorm*eps)) then
c            full_reorth = .true.

            debito=.true.
            ierr = j
         endif            

         B(j,2) = beta
C           print *, "before zsafescal, j = ", j, "iu = ", iu
C           do ips = iu, iu + m
C             if (zwork(ips).ne.(0.0d0,0.0d0)) then
C                PRINT *, "zwork(", ips, "):",  zwork(ips)
C             end if
C           end do
C           print *, "------"


c         write (*,*) 'beta1 = ',beta
C          print *, "beta = ", beta
         if (beta.ne.zero .and. beta.ne.one) then
            call donothing()
            call zsafescal(m,beta,zwork(iu))
         endif
C           print *, "before zcopy, j = ", j
C           do ips = 1, m
C             do jps = 1, kmax + 1
C                 if (U(ips,jps).ne.(0.0d0,0.0d0)) then
C                    PRINT *, "U(", ips, jps, "):",  U(ips,jps)
C                 end if
C            end do
C           end do
C           print *, "------"
C           print *, "before zcopy, j = ", j, "iu = ", iu
C           do ips = iu, iu + m
C             if (zwork(ips).ne.(0.0d0,0.0d0)) then
C                PRINT *, "zwork(", ips, "):",  zwork(ips)
C             end if
C           end do
C           print *, "------"

         call zcopy(m,zwork(iu),1,U(1,j+1),1)

 

         rnorm = beta
         call second(t2)
      enddo
 9999 doption(3) = anorm      
      call second(t2)
      tlanbpro = tlanbpro + (t2-t1)
 100  format(1i6,6e12.3)
 101  format(1i6,4e12.3)

C       print *, "zlanbprow done"
C       print *, "printing non-zero Us, j = ", j
C       do ips = 1, m
C         do jps = 1, kmax + 1
C             if (U(ips,jps).ne.(0.0d0,0.0d0)) then
C                PRINT *, "U(", ips, jps, "):",  U(ips,jps)
C             end if
C        end do
C       end do
C       print *, "------"


C       print *, "printing non-zero Vs, j = ", j
C       do ips = 1, n
C         do jps = 1, kmax
C             if (V(ips,jps).ne.(0.0d0,0.0d0)) then
C                PRINT *, "V(", ips, jps, "):",  V(ips,jps)
C             end if
C        end do
C       end do
C       print *, "------"


C       do ips = 1, ldb
C         do jps = 1, 2
C                PRINT *, "B(", ips, jps, "):",  B(ips,jps)
C        end do
C       end do
C       print *, "------"

C       print *, "printing work"
C       do ips = 1, 20000
C          PRINT *, "work(", ips, "):",  work(ips)
C       end do
C       print *, "------"

C      call FLUSH()

      return
      end

c
c**********************************************************************
c

      subroutine dset_mu(k,mu,index,val)
c     %-----------%
c     | Arguments |
c     %-----------%
      implicit none
      integer k,index(*)
      double precision mu(*), val

c     %-----------------%
c     | Local variables |
c     %-----------------%
      integer i,j,p,q

      i=1
      do while(index(i).le.k .and. index(i).gt.0)
         p = index(i)
         q = index(i+1)
         do j=p,q
            mu(j) = val
         enddo

         i = i+2
      enddo      
      end
c
c**********************************************************************
c
      subroutine dcompute_int(mu,j,delta,eta,index)
c     %-----------%
c     | Arguments |
c     %-----------%
      implicit none
      include 'stat.h'
      integer j,index(*)
      double precision mu(*)
      double precision delta,eta

c     %-----------------%
c     | Local variables |
c     %-----------------%
      integer i,k,s,ip
      real t1,t2
      integer ips, jps



      call second(t1)

      if (delta.lt.eta) then
         return
      endif

c      write (*,*) 'Enter compute_int, j=',j
C       print *, "begin dcompute_int, j = ", j
C       print *, "delta = ", delta, ", eta = ", eta
      jps = 0
      do ips = 1, 50
        if (mu(ips).eq.0) then
            if (jps.eq.0) then
                jps = ips
            endif
        endif
      end do
C       do ips = 1, jps
C         print *, "mu(", ips, ") = ", mu(ips)
C       end do


      ip = 0
      index(1) = 0
      i=0
      do while(i.lt.j)
c     find the next mu(k), k>i where abs(mu(k)) > delta
         do k=i+1,j
c            write(*,*) i,k,abs(mu(k))
            if (abs(mu(k)).gt.delta) goto 10
         enddo         
         goto 40

c     find smallest i<k such that for all j=i,..,k, m(j) >= eta
 10      do s=k,max(i,1),-1
            if (abs(mu(s)).lt.eta) goto 20
         enddo
 20      continue
C          print *, "dcompute_int: k = ", k, "s = ", s, 
C      c            "i = ", i
C         ip= ip+1
         ip= ip+1
         index(ip) = s+1
         do i=s+1,j            
            if (abs(mu(i)).lt.eta) goto 30
         enddo         
30       continue
C          print *, "dcompute_int: k = ", k, "s = ", s, 
C      c            "i = ", i

         ip= ip+1
         index(ip) = i-1
      enddo
 40   ip = ip+1
      index(ip) = j+1
c      write (*,*) 'i  index(i)'
c      do i=1,j
c         write(*,*) i,index(i)
c         if (index(i).gt.j) goto 50
c      enddo
c 50   write (*,*) 'Exit compute_int, ip=',ip
C       print *, "Exiting dcompute_int, ip = ", ip
C       do ips=1, 10
C         print *, "index(", ips, ") = ", index(ips)
C       end do

C       print *, "end dcompute_int"
C       do ips = 1, 5
C         print *, "index(", ips, ") = ", index(ips)
C       end do


      call second(t2)
      tintv = tintv + (t2-t1)
      end
c
c**********************************************************************
c
      subroutine dupdate_mu(mumax,mu,nu,j,alpha,beta,anorm)
c     %-----------%
c     | Arguments |
c     %-----------%
      implicit none
      include 'stat.h'
      integer j,formula
      parameter(formula=1)
      double precision mumax,eps1,eps,anorm
      double precision mu(*),nu(*),alpha(*),beta(*)

c     %------------%
c     | Parameters |
c     %------------%
      double precision one, zero, FUDGE
      parameter(one = 1.0d0, zero = 0.0d0, FUDGE = 1.01)

c     %-----------------%
c     | Local variables |
c     %-----------------%
      double precision d
      integer k
      real t1,t2
      
c     %--------------------%
c     | External Functions |
c     %--------------------%
      double precision dlamch,dlapy2
      external dlamch,dlapy2

c      write (*,*) 'Enter update_mu'
      call second(t1)
      eps = dlamch('e')
      eps1 = 100*eps
      if (formula.eq.1) then
         if (j.eq.1) then
            d = eps1*(dlapy2(alpha(j), beta(j)) + alpha(1)) + eps1*anorm
            mu(1) = eps1/beta(1)
            mumax = abs(mu(1))
         else
            mu(1) = alpha(1)*nu(1)-alpha(j)*mu(1)
C             print *, "dupdate_mu: alpha(1) = ", alpha(1)
C             print *, "dupdate_mu: nu(1) = ", nu(1)
C             print *, "dupdate_mu: alpha(j) = ", alpha(j)
C             print *, "dupdate_mu: mu(1) = ", mu(1)

            d = eps1*(dlapy2(alpha(j), beta(j)) + alpha(1)) + eps1*anorm
            mu(1) = (mu(1) + dsign(d,mu(1))) / beta(j)
            mumax = abs(mu(1))
            do k=2,j-1
               mu(k) = alpha(k)*nu(k) +beta(k-1)*nu(k-1)-alpha(j)*mu(k)
               d = eps1*(dlapy2(alpha(j), beta(j)) + 
     c              dlapy2(alpha(k), beta(k-1))) + eps1*anorm
               mu(k) = (mu(k) + dsign(d,mu(k))) / beta(j)
               mumax = max(mumax,abs(mu(k)))
            enddo
            mu(j) = beta(j-1)*nu(j-1)
            d = eps1*(dlapy2(alpha(j), beta(j)) + 
     c           dlapy2(alpha(j), beta(j-1))) + eps1*anorm
            mu(j) = (mu(j) + sign(d,mu(j))) / beta(j)
            mumax = max(mumax,abs(mu(j)))
         endif
         mu(j+1) = one
      else
         if (j.eq.1) then
            mu(1) = eps1/beta(1)
            mumax = abs(mu(1))
         else
            mu(1) = alpha(1)*nu(1)-alpha(j)*mu(1)
            mu(1) = (mu(1) + dsign(eps1,mu(1))) / beta(j)
            mumax = abs(mu(1))
            do k=2,j-1
               mu(k) = alpha(k)*nu(k) + beta(k-1)*nu(k-1)-alpha(j)*mu(k)
               mu(k) = (mu(k) + dsign(eps1,mu(k))) / beta(j)
               mumax = max(mumax,abs(mu(k)))
            enddo
            mu(j) = beta(j-1)*nu(j-1)
            mu(j) = (mu(j) + sign(eps1,mu(j))) / beta(j)
            mumax = max(mumax,abs(mu(j)))
         endif
         mu(j+1) = one
      endif
      call second(t2)
      tupdmu = tupdmu + (t2-t1)
c      write (*,*) 'Exit update_mu'
      end
c
c**********************************************************************
c

      subroutine dupdate_nu(numax,mu,nu,j,alpha,beta,anorm)
c     %-----------%
c     | Arguments |
c     %-----------%
      implicit none
      include 'stat.h'
      integer j,formula
      parameter(formula=1)
      double precision numax,eps1,anorm
      double precision mu(*),nu(*),alpha(*),beta(*)

c     %------------%
c     | Parameters |
c     %------------%
      double precision one, zero, FUDGE
      parameter(one = 1.0d0, zero = 0.0d0, FUDGE = 1.01)

c     %-----------------%
c     | Local variables |
c     %-----------------%
      double precision d,eps
      integer k
      real t1,t2

c     %--------------------%
c     | External Functions |
c     %--------------------%
      double precision dlamch,dlapy2
      external dlamch,dlapy2

c      write (*,*) 'Enter update_nu'
      call second(t1)
      eps = dlamch('e')
      eps1 = 100*eps
      if (formula.eq.1) then
         if (j.gt.1) then
            numax = zero
            do k=1,j-1
C                 print *, "dupdate_nu: k = ", k
C                 print *, "dupdate_nu: beta(k) = ", beta(k)
C                 print *, "dupdate_nu: mu(k+1) = ", mu(k+1)
C                 print *, "dupdate_nu: alpha(k) = ", alpha(k)
C                 print *, "dupdate_nu: mu(k) = ", mu(k)
C                 print *, "dupdate_nu: beta(j - 1) = ", beta(j - 1)
C                 print *, "dupdate_nu: nu[k] = ", nu(k)
               nu(k) = beta(k)*mu(k+1) + alpha(k)*mu(k) -beta(j-1)*nu(k)
               d = eps1*(dlapy2(alpha(k),beta(k)) +
     c              dlapy2(alpha(j),beta(j-1))) + eps1*anorm
               nu(k) = (nu(k) + dsign(d,nu(k))) / alpha(j)
               numax = max(numax,abs(nu(k)))
            enddo
            nu(j) = one
         endif
      else
         if (j.gt.1) then
            numax = zero
            do k=1,j-1
               nu(k) = beta(k)*mu(k+1) + alpha(k)*mu(k) -beta(j-1)*nu(k)
               nu(k) = (nu(k) + dsign(eps1,nu(k))) / alpha(j)
               numax = max(numax,abs(nu(k)))
            enddo
            nu(j) = one
         endif
      endif
      call second(t2)
      tupdnu = tupdnu + (t2-t1)
c      write (*,*) 'Exit update_nu'
      end

      subroutine donothing()

c     Added by the Vespa team to address ticket #49:
c     http://scion.duhs.duke.edu/vespa/analysis/ticket/49
c
c     It writes "" to /dev/null which should do nothing but apparently
c     it has an important side effect. (Read the ticket.)

      character*255 envname, envvalue, bitbucket
      integer envlength, i

c     Determine the location of the bit bucket based on OS. The
c     OS environment variable exists on Windows but not elsewhere.
      envname = "OS"
      call get_environment_variable(envname, envvalue, envlength)

c     Convert OS name (if it exists) to lower case
      do i=1, envlength
         if (LGE(envvalue(i:i),'A') .AND. LLE(envvalue(i:i),'Z')) THEN
            envvalue(i:i) = ACHAR(IACHAR(envvalue(i:i)) + 32)
         endif
      enddo

      i = index(envvalue, "windows")
      if (i.ne.0) then
c        We're on Windows
         bitbucket = "NUL"
      else
c        We're on *nix
         bitbucket = "/dev/null"
      endif

      open(10, file=bitbucket, status="old")
      write(10, "()")
      close(10)

      end


      subroutine zwrite(filename, n, a_vector)
          character filename*(*)
          integer n, ips
          complex*16 a_vector(*)
          character*1 TAB
          parameter (TAB=char(9))

 67       FORMAT (F30.17, A1, F30.17)
          OPEN(UNIT=42, FILE=filename)

          do ips =1, n
            write(42, 67) REALPART(a_vector(ips)), TAB, 
     c                    IMAGPART(a_vector(ips))
          end do

          CLOSE(42)

      end


